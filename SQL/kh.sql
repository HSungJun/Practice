...
FROM
    employee e2
    INNER JOIN department d ON e2.dept_code = d.dept_id
    INNER JOIN employee e1 ON e2.manager_id = e1.emp_id
WHERE
    e1.emp_name = '선동일'; 
...
--id = 식별자 
-- id = 컬럼명, number = 자료형
--varchar - 최대 사이즈를 넣어주어야 함 / 최대 사이즈보다 적게 입력시 나머지값 반환
-- char - 최대값 지정후 입력시 변하지않는다.
...
--insert into cafe_menu (컬럼명....) values (순서대로 데이터)
...
--DML : update - table객체에 저장된 데이터를 수정하는 SQL 
...
-- 한개의 컬럼만 지우는 것 불가능. null을 삽입하는것은 업데이트로 가능.

delete from cafe_menu where id = 1004; 


--DML : insert, select, update, delete >  애플리케이션에서의 CRUD

--DDL : drop - oracle 내부에 존재하는 객체를 삭제하는 SQL
drop table cafe_menu;


--테이블에 규칙 생성
--DDL : create 2 
--          not null 제약조건       -   컬럼에 null값 허용 x
--          primary key 제약조건    -   특정 컬럼을 각 행을 구분하는 식별자로 지정 (null, 중복 방지)
--          unique 제약조건         -   컬럼에 중복 방지를 적용 (null 허용, 중복 방지)
--          check 제약조건          -   컬럼에 입력이 허용되는 값을 지정
--          foreign key 제약조건    -   테이블간 상호 데이터 무결성을 검증

--not null
-- 특정 컬럼을 각 행을 구분하는 식별자로 지정 (null ,중복 방지)
create table cafe_menu(
id number not null,
...
update cafe_menu set price=null where id  = 1001;
insert into cafe_menu values (1002, 'Cafe Ltte',3000,'N');

select * from cafe_menu;

--check : 제약조건 - 컬럼에 입력이 허용되는 값을 지정함.
...
insert into product values (1002,'GalayS23',2000000 );
...
--sid (판매기록id), pid(판매항목 id), sdate(판매날짜)
-- 외래키 - references 는 unique, primary key로 설정된 값을 참조하여 가져오는 문법
-- 불러온 값은 parenttable과 childtable로 구분된다.
create table sales_history(
    sid number primary key,
    pid references product(id) on delete cascade,
    sdate timestamp not null
);
--on delete cascade product 값 참조한 경우 사용할때에 
-- parent 키를 지웠을 때에 연쇠적으로 지워짐.


insert into sales_history values (10001, 1001, sysdate);
insert into sales_history values (10002, 1002, sysdate);
insert into sales_history values (10003, 1003, sysdate);
insert into sales_history values (10004, 1004, sysdate);

delete from product where id = 1001;
--외래키 foreign key 의 경우 child record가 있으면 parent record 삭제불가능

drop table sales_history;

delete from product where id = 1002;
-- on delete cascade 로 인해 부모 자식 레코드가 함께 삭제되게된다.
...
-- on delete set null 의 경우 부모데이터 삭제시 null 값으로 유지하겠다라는 의미
...
------------------------------------------------------------------
------------------------------------------------------------------
--sequence 객체 - 사용자가 지정한 패턴에 따라 자동증가 또는 감소하는 값을 관리하는 객체


--create sequence (이름) start with (시작점) increment by(증감량) nomaxvalue nocache;
create sequence product_seq start with 1002 
...
select * from cafe;
--컬럼 추가
alter table cafe add (menu_size varchar(10));

--컬럼 삭제
alter table cafe drop column takeout;
select * from cafe;

-- null값을 허용하지 않는다고 하였으나 데이터입력을 하지 않아 null값이 되므로
-- 만족하지않는다. 
alter table cafe add (menu_size varchar(10) not null);
...
desc cafe;
select * from cafe;
-- alter table (테이블명) modify (변경데이터명) (수정사항);
alter table cafe modify name varchar(30) not null;
...
select * from user_tables; --테이블 검색
select * from user_constraints where table_name = 'CAFE'; -- 제약조건 검색

delete from user_constraints where constraint_name='SYS_C007037';
--권한이 없어 삭제 불가능

alter table cafe drop constraint SYS_C007037;
-- 제약조건 삭제.

alter table cafe drop constraint sys_c007036;

alter table cafe add primary key(id); -- 찾기 힘듬
alter table cafe add constraint cafe_id_pk primary key(id);
-- 우리가 정한 이름으로 들어가도록 설정

--유니크 및 체크제약 추가
alter table cafe add constraint cafe_name_uk unique(name);
alter table cafe add constraint cafe_inIce_chk check(isIce in ('Y','N'));

--처음 컬럼 생성시 제약조건에 이름 추가
drop table cafe;
create table cafe (
...
---------------------------------------------------
-- 감소하게 할 경우 increment by (음수) 를 이용한다. decrement 사용x
...
-- start with 값은 수정할 수 없다.
alter sequence cafe_seq cycle;

--계정 패스워드 변경가능
--alter user kh identified by tset;
...
-- employee_dev 라는 테이블을 추출생성하였음.

create user dev identified by dev;
--계정생성 불가 - admin 에서 실행하며 권한부여도 해주어야함
--grant resource, connect to dev; 

--employee 데이터가 업데이트 되어도 떼어낸 employee_dev 의 업데이트는 수동적이기 때문에
--view 를 통한 특정요소만을 보여주는 명령어를 사용해서 실시간으로 볼 수 있도록 한다.

--create user dev identified by dev; 
-- admin 계정으로 dev 계정을 생성하고 권한을 부여함.
--grant resource, connect to dev;

select * from employee;
--선동일 > 선동이
--employee_dev 와 view 의 차이를 보기 위해서 실행
update employee set emp_name = '선동이' where emp_id = 200;
commit;


-- grantor 권한부여자 gtantee 권한받은자
select * from user_tab_privs; -- 권한확인
...
--> insert delete 등은 DB가 아닌 transaction에 저장됨
--rollback : transaction에 저장된 delete 나 insert 등으로 인한 
--querry를 취소하고 가장 최근에 commit이 수행된 시점 으로 돌아감.
delete from employee;
select * from employee;
rollback;
-- commit : transaction 에 저장된 모든 쿼리를 DB에 실제로 반영하는 명령어
--drop 은 DDL 로서 자동으로 commit이 실행되므로 rollback이 불가능

-- transaction 존재 이유 - 안전한 쿼리 운용, 작업의 원자성을 만들어 내기 위해,데이터 격리성,
-- 작업의 원자성 = 개발자가 이용하는 성질 / 데이터 격리성 = 개발자가 주의해야하는 성질
-- 계정 접속세션이 각각 다르며 계정마다 transaction을 보유하고 있음. 
...
--한쪽 세션에서 transaction에 등록해놓을경우 다른 세션에서 '락'에 걸림
-- 
